...
const Upcoming = () => {
  ...
  const fetchData = async () => {
    await axios
      .get(
        "https://api.themoviedb.org/3/movie/upcoming?api_key=a2127362ead2634e824bdf7cd0478223"
      )
      .then((response) => {
  ...
  return (
    <Layout>
      <div className="pt-28 ml-2">
        <h1 className="text-4xl font-bold text-slate-200">Up Coming</h1>
      </div>
      <div className="py-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 place-items-center">
        {coming.map((coming) => (
          <Suspense key={coming.id} fallback={<Skeletoncard />}>
            <Card
              key={coming.id}
              ...
              vote={coming.vote_count}
              onClick={() => handleFav(coming)}
            />
          </Suspense>
        ))}
      </div>
